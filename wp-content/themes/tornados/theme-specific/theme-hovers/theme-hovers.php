...
if ( ! function_exists( 'tornados_hovers_theme_setup3' ) ) {
	add_action( 'after_setup_theme', 'tornados_hovers_theme_setup3', 3 );
	function tornados_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		tornados_storage_set_array_after(
			'options', 'border_radius', array(
				'button_hover' => array(
					'title'   => esc_html__( "Button's hover", 'tornados' ),
					'desc'    => wp_kses_data( __( 'Select hover effect to decorate all theme buttons', 'tornados' ) ),
					'std'     => 'default',
					'options' => array(
						'default'      => esc_html__( 'Fade', 'tornados' ),
					),
					'type'    => 'select',
				),
				'image_hover'  => array(
					'title'    => esc_html__( "Image's hover", 'tornados' ),
					'desc'     => wp_kses_data( __( 'Select hover effect to decorate all theme images', 'tornados' ) ),
					'std'      => 'simple',
					'override' => array(
						'mode'    => 'page',
						'section' => esc_html__( 'Content', 'tornados' ),
					),
					'options'  => tornados_get_list_hovers(),
					'type'     => 'select',
				),
			)
		);
...
if ( ! function_exists( 'tornados_hovers_theme_setup9' ) ) {
	add_action( 'after_setup_theme', 'tornados_hovers_theme_setup9', 9 );
	function tornados_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts', 'tornados_hovers_frontend_scripts', 1010 );
		add_action( 'wp_enqueue_scripts', 'tornados_hovers_frontend_styles', 1100 );
		add_action( 'wp_enqueue_scripts', 'tornados_hovers_responsive_styles', 2000 );
		add_filter( 'tornados_filter_localize_script', 'tornados_hovers_localize_script' );
		add_filter( 'tornados_filter_merge_scripts', 'tornados_hovers_merge_scripts' );
		add_filter( 'tornados_filter_merge_styles', 'tornados_hovers_merge_styles' );
		add_filter( 'tornados_filter_merge_styles_responsive', 'tornados_hovers_merge_styles_responsive' );
		add_filter( 'tornados_filter_get_css', 'tornados_hovers_get_css', 10, 2 );
	}
}

// Enqueue hover styles and scripts
if ( ! function_exists( 'tornados_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'tornados_hovers_frontend_scripts', 1010 );
	function tornados_hovers_frontend_scripts() {
		if ( tornados_is_on( tornados_get_theme_option( 'debug_mode' ) ) ) {
			$tornados_url = tornados_get_file_url( 'theme-specific/theme-hovers/theme-hovers.js' );
			if ( '' != $tornados_url ) {
				wp_enqueue_script( 'tornados-hovers', $tornados_url, array( 'jquery' ), null, true );
			}
		}
	}
}
...
if ( ! function_exists( 'tornados_hovers_frontend_styles' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'tornados_hovers_frontend_styles', 1100 );
	function tornados_hovers_frontend_styles() {
		if ( tornados_is_on( tornados_get_theme_option( 'debug_mode' ) ) ) {
			$tornados_url = tornados_get_file_url( 'theme-specific/theme-hovers/theme-hovers.css' );
			if ( '' != $tornados_url ) {
				wp_enqueue_style( 'tornados-hovers', $tornados_url, array(), null );
			}
		}
	}
}
...
if ( ! function_exists( 'tornados_hovers_responsive_styles' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'tornados_hovers_responsive_styles', 2000 );
	function tornados_hovers_responsive_styles() {
		if ( tornados_is_on( tornados_get_theme_option( 'debug_mode' ) ) ) {
			$tornados_url = tornados_get_file_url( 'theme-specific/theme-hovers/theme-hovers-responsive.css' );
			if ( '' != $tornados_url ) {
				wp_enqueue_style( 'tornados-hovers-responsive', $tornados_url, array(), null );
			}
		}
	}
}
...
if ( ! function_exists( 'tornados_hovers_merge_styles' ) ) {
	//Handler of the add_filter( 'tornados_filter_merge_styles', 'tornados_hovers_merge_styles' );
	function tornados_hovers_merge_styles( $list ) {
...
if ( ! function_exists( 'tornados_hovers_merge_styles_responsive' ) ) {
	//Handler of the add_filter( 'tornados_filter_merge_styles_responsive', 'tornados_hovers_merge_styles_responsive' );
	function tornados_hovers_merge_styles_responsive( $list ) {
...
if ( ! function_exists( 'tornados_hovers_localize_script' ) ) {
	//Handler of the add_filter( 'tornados_filter_localize_script','tornados_hovers_localize_script' );
	function tornados_hovers_localize_script( $arr ) {
		$arr['button_hover'] = tornados_get_theme_option( 'button_hover' );
		return $arr;
	}
}

// Merge hover effects to the single js
if ( ! function_exists( 'tornados_hovers_merge_scripts' ) ) {
	//Handler of the add_filter( 'tornados_filter_merge_scripts', 'tornados_hovers_merge_scripts' );
	function tornados_hovers_merge_scripts( $list ) {
...
if ( ! function_exists( 'tornados_hovers_add_icons' ) ) {
	function tornados_hovers_add_icons( $hover, $args = array() ) {
		...
		if ( in_array( $hover, array( 'icons', 'zoom' ) ) ) {
			...
			?>
			<div class="icons">
				<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php tornados_show_layout($target); ?> aria-hidden="true" class="icon-link
									<?php
			...
			<div class="icons">
						...
						apply_filters(
							...
							'<a rel="nofollow" href="' . esc_url( $product->add_to_cart_url() ) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ( 'shop_buttons' == $hover ? ( $product->is_type( 'variable' ) ? esc_html__( 'Select options', 'tornados' ) : esc_html__( 'Buy now', 'tornados' ) ) : '' )
										. '</a>',
							$product
						)
					);
				...
				if ( 'shop_buttons' == $hover ) {
					if ( is_object( $args['cat'] ) ) {
						esc_html_e( 'View products', 'tornados' );
					} else {
						esc_html_e( 'Details', 'tornados' );
					}
				}
				?>
				</a>
			...
			// Hover style 'Icon'
			?>
			<div class="icons"><a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php tornados_show_layout($target); ?> aria-hidden="true" class="icon-search-alt"></a></div>
			<?php

		} elseif ( 'dots' == $hover ) {
			// Hover style 'Dots'
			?>
			<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php tornados_show_layout($target); ?> aria-hidden="true" class="icons"><span></span><span></span><span></span></a>
			<?php
			...
			<div class="post_info">
				<div class="post_info_back">
					...
					if ( ! empty( $post_link ) ) {
						?>
							<a href="<?php echo esc_url( $post_link ); ?>" <?php tornados_show_layout($target); ?>>
							<?php
					}
						the_title();
					if ( ! empty( $post_link ) ) {
			...
			// Hover style empty
			?>
			<a href="<?php echo esc_url( $post_link ); ?>" <?php tornados_show_layout($target); ?> aria-hidden="true" class="icons"></a>
			<?php
		}
	}
}
...
// Add styles into CSS
if ( ! function_exists( 'tornados_hovers_get_css' ) ) {
	//Handler of the add_filter( 'tornados_filter_get_css', 'tornados_hovers_get_css', 10, 2 );
	function tornados_hovers_get_css( $css, $args ) {
