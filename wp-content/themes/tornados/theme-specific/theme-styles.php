...
if ( ! function_exists( 'tornados_customizer_get_css' ) ) {

	function tornados_customizer_get_css( $args = array() ) {
		...
		if ( $fonts ) {
			...
			$fonts = tornados_customizer_add_theme_fonts( $fonts );
...
CSS;
			$rez          = apply_filters( 'tornados_filter_get_css', $rez, array( 'fonts' => $fonts ) );
			$css['fonts'] = $rez['fonts'];
		}
		...
		if ( $vars ) {
			...
			// Make theme-specific fonts rules
			$vars = tornados_customizer_add_theme_vars( $vars );
			extract( $vars );
...
CSS;
			$rez          = apply_filters( 'tornados_filter_get_css', $rez, array( 'vars' => $vars ) );
			$css['vars']  = $rez['vars'];
		}
		...
		//--------------------------------------
		if ( false !== $colors ) {
			$schemes = empty( $scheme ) ? array_keys( tornados_get_sorted_schemes() ) : array( $scheme );
			...
			if ( count( $schemes ) > 0 ) {
				...
				foreach ( $schemes as $s ) {
					// Prepare colors
					if ( empty( $scheme ) ) {
						$colors = tornados_get_scheme_colors( $s );
					}
...
.tornados_tabs .tornados_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
div.esg-filter-wrapper .esg-filterbutton.selected > span,
.mptt-navigation-tabs li.active a,
.tornados_tabs .tornados_tabs_titles li.ui-state-active a {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
...
.sheme_self.sidebar .tornados_tabs .tornados_tabs_titles li a {
	color: {$colors['alter_dark']};
	background-color: {$colors['alter_bg_hover']};
}
.sheme_self.sidebar div.esg-filter-wrapper .esg-filterbutton > span:hover,
.sheme_self.sidebar .mptt-navigation-tabs li a:hover,
.sheme_self.sidebar .tornados_tabs .tornados_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['alter_link']};
}
...
.sheme_self.sidebar .tornados_tabs .tornados_tabs_titles li.ui-state-active a {
...
.scheme_self.footer_wrap .widget_archive ul li,
.scheme_self.footer_wrap .widget_categories li {
	color: {$colors['alter_dark']};
}
...
CSS;
					...
					$rez            = apply_filters(
						'tornados_filter_get_css', $rez, array(
							'colors' => $colors,
							'scheme' => $s,
						)
					);
		...
		return apply_filters( 'tornados_filter_prepare_css', $css_str, $remove_spaces );
	}
}
