...
------------------------------------------------------------------------------- */
if ( ! function_exists( 'tornados_trx_addons_theme_specific_components' ) ) {
	add_filter( 'trx_addons_filter_components_editor', 'tornados_trx_addons_theme_specific_components' );
	function tornados_trx_addons_theme_specific_components( $enable = false ) {
		return false;
	}
}

if ( ! function_exists( 'tornados_trx_addons_theme_specific_setup1' ) ) {
	add_action( 'after_setup_theme', 'tornados_trx_addons_theme_specific_setup1', 1 );
	function tornados_trx_addons_theme_specific_setup1() {
		if ( tornados_exists_trx_addons() ) {
			add_filter( 'trx_addons_cv_enable', 'tornados_trx_addons_cv_enable' );
			add_filter( 'trx_addons_demo_enable', 'tornados_trx_addons_demo_enable' );
			add_filter( 'trx_addons_filter_edd_themes_market', 'tornados_trx_addons_edd_themes_market_enable' );
			add_filter( 'trx_addons_api_list', 'tornados_trx_addons_api_list' );
			add_filter( 'trx_addons_cpt_list', 'tornados_trx_addons_cpt_list' );
			add_filter( 'trx_addons_sc_list', 'tornados_trx_addons_sc_list' );
			add_filter( 'trx_addons_widgets_list', 'tornados_trx_addons_widgets_list' );
		}
	}
}

if ( ! function_exists( 'tornados_trx_addons_theme_specific_setup' ) ) {
	add_action( 'after_setup_theme', 'tornados_trx_addons_theme_specific_setup' );
	function tornados_trx_addons_theme_specific_setup() {
		if ( tornados_exists_trx_addons() ) {
			if ( ! is_admin() ) {
				add_action( 'tornados_action_before_post_meta', 'tornados_trx_addons_action_before_post_meta' );
			}
		}
	}
}
...
// CV
if ( ! function_exists( 'tornados_trx_addons_cv_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_cv_enable', 'tornados_trx_addons_cv_enable');
	function tornados_trx_addons_cv_enable( $enable = false ) {
		// To do: return false if theme not use CV functionality
		return TORNADOS_THEME_FREE
					? false     // Free version
					: true;     // Pro version
	}
}
...
// Demo mode
if ( ! function_exists( 'tornados_trx_addons_demo_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_demo_enable', 'tornados_trx_addons_demo_enable');
	function tornados_trx_addons_demo_enable( $enable = false ) {
...
// EDD Themes market
if ( ! function_exists( 'tornados_trx_addons_edd_themes_market_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_edd_themes_market', 'tornados_trx_addons_edd_themes_market_enable')) {
	function tornados_trx_addons_edd_themes_market_enable( $enable = false ) {
...
// API
if ( ! function_exists( 'tornados_trx_addons_api_list' ) ) {
	//Handler of the add_filter('trx_addons_api_list',	'tornados_trx_addons_api_list');
	function tornados_trx_addons_api_list( $list = array() ) {
		// To do: Enable/Disable Third-party plugins API via add/remove it in the list


        $list[ 'sportspress' ] = array(
            'title' => esc_html__('SportsPress – Sports Club & League Manager', 'tornados'),
            // Always enabled!!!
            'std' => 1,
            'hidden' => false
        );
...
// CPT
if ( ! function_exists( 'tornados_trx_addons_cpt_list' ) ) {
	//Handler of the add_filter('trx_addons_cpt_list',	'tornados_trx_addons_cpt_list');
	function tornados_trx_addons_cpt_list( $list = array() ) {
...
// Shortcodes
if ( ! function_exists( 'tornados_trx_addons_sc_list' ) ) {
	//Handler of the add_filter('trx_addons_sc_list',	'tornados_trx_addons_sc_list');
	function tornados_trx_addons_sc_list( $list = array() ) {
        ...
        $list['blogger']['templates']['default']['classic_meta'] = array(
            'title'  => esc_html__('Classic Grid (meta)', 'tornados'),
            'layout' => array(
                'featured' => array(
                ),
                'content' => array(
        ...
        $list['blogger']['templates']['default']['classic_meta_simple'] = array(
            'title'  => esc_html__('Simple Grid (meta)', 'tornados'),
            'layout' => array(
                'featured' => array(
                ),
                'content' => array(
        ...
        $list['blogger']['templates']['default']['over_bottom_meta'] = array(
            'title'  => esc_html__('Info over image (top meta)', 'tornados'),
            'layout' => array(
                'featured' => array(
                    'bc' => array(
                        'meta', 'title'
...
// Widgets
if ( ! function_exists( 'tornados_trx_addons_widgets_list' ) ) {
	//Handler of the add_filter('trx_addons_widgets_list',	'tornados_trx_addons_widgets_list');
	function tornados_trx_addons_widgets_list( $list = array() ) {
        ...
        $list['recent_news']['layouts_sc']['news-plain'] = esc_html__('Plain',	'tornados');
        $list['recent_news']['layouts_sc']['news-line'] = esc_html__('Line',	'tornados');
...
if ( ! function_exists( 'tornados_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'tornados_trx_addons_menu_cache' );
	function tornados_trx_addons_menu_cache( $list = array() ) {
...
if ( ! function_exists( 'tornados_trx_addons_localize_script' ) ) {
	...
	function tornados_trx_addons_localize_script( $arr ) {
		$arr['alter_link_color'] = tornados_get_scheme_color( 'alter_link' );
		return $arr;
	}
}
...
if ( ! function_exists( 'tornados_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'tornados_trx_addons_sc_type', 10, 2 );
	function tornados_trx_addons_sc_type( $list, $sc ) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
		if ( 'trx_sc_blogger' == $sc ) {
			$list = tornados_array_merge( $list, tornados_get_list_blog_styles( false, 'sc' ) );

		}

        if ( 'trx_sc_blogger' == $sc ) {
            $list['line'] = esc_html__('Line-up', 'tornados');
        }
        if ( 'trx_sc_button' == $sc ) {
            $list['default circle'] = esc_html__('No title, circle', 'tornados');
        }
...
if ( ! function_exists( 'tornados_trx_addons_sc_prepare_atts' ) ) {
	add_filter( 'trx_addons_filter_sc_prepare_atts', 'tornados_trx_addons_sc_prepare_atts', 10, 2 );
	function tornados_trx_addons_sc_prepare_atts( $atts, $sc ) {
		if ( 'trx_sc_blogger' == $sc ) {
			$list = tornados_get_list_blog_styles( false, 'sc' );
			if ( isset( $list[ $atts['type'] ] ) ) {
				$custom_type = '';
				if ( strpos( $atts['type'], 'blog-custom-' ) === 0 ) {
					$blog_id = tornados_get_custom_blog_id( $atts['type'] );
...
if ( ! function_exists( 'tornados_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'tornados_trx_addons_sc_atts', 10, 2 );
	function tornados_trx_addons_sc_atts( $atts, $sc ) {
...
if ( ! function_exists( 'tornados_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'tornados_trx_addons_sc_map', 10, 2 );
	function tornados_trx_addons_sc_map( $params, $sc ) {
		...
		$param = array(
			...
			'heading'          => esc_html__( 'Color style', 'tornados' ),
			'description'      => wp_kses_data( __( 'Select color style to decorate this block', 'tornados' ) ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label'      => true,
			'value'            => array_flip( tornados_get_list_sc_color_styles() ),
			'type'             => 'dropdown',
		);
		...
		if ( 'trx_sc_blogger' == $sc ) {
			...
			$param      = array(
				'param_name'       => 'hover',
				'heading'          => esc_html__( 'Image hover', 'tornados' ),
				'description'      => wp_kses_data( __( 'Select hover effect for the featured image', 'tornados' ) ),
				'edit_field_class' => 'vc_col-sm-4',
				'value'            => array_flip( tornados_get_list_hovers( true ) ),
				'dependency'       => array(
					'element' => 'type',
...
if ( ! function_exists( 'tornados_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'tornados_trx_addons_sc_output', 10, 4 );
	function tornados_trx_addons_sc_output( $output, $sc, $atts, $content ) {
		...
		if ( in_array( $sc, array( 'trx_sc_action' ) ) ) {
			...
			if ( ! empty( $atts['color_style'] ) && ! tornados_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'tornados_trx_addons_sc_item_link_classes' ) ) {
	add_filter( 'trx_addons_filter_sc_item_link_classes', 'tornados_trx_addons_sc_item_link_classes', 10, 3 );
	function tornados_trx_addons_sc_item_link_classes( $class, $sc, $atts=array() ) {
		if ( 'sc_button' == $sc ) {
			if ( ! empty( $atts['color_style'] ) && ! tornados_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'tornados_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'tornados_trx_addons_sc_item_title_tag' );
	function tornados_trx_addons_sc_item_title_tag( $tag = '' ) {
		return 'h1' == $tag ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( ! function_exists( 'tornados_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'tornados_trx_addons_sc_item_button_args', 10, 3 );
	function tornados_trx_addons_sc_item_button_args( $args, $sc, $sc_args ) {
...
if ( ! function_exists( 'tornados_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title', 'tornados_trx_addons_slider_title', 10, 2 );
	function tornados_trx_addons_slider_title( $title, $data ) {
		$title = '';
		if ( ! empty( $data['title'] ) ) {
...
if ( ! function_exists( 'tornados_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles', 'tornados_trx_addons_sc_googlemap_styles' );
	function tornados_trx_addons_sc_googlemap_styles( $list ) {
		$list['dark'] = esc_html__( 'Dark', 'tornados' );
		return $list;
	}
}

// Show reactions in the single posts
if ( ! function_exists( 'tornados_trx_addons_action_before_post_meta' ) ) {
	//Handler of the add_action( 'tornados_action_before_post_meta', 'tornados_trx_addons_action_before_post_meta');
	function tornados_trx_addons_action_before_post_meta() {
		if ( tornados_exists_trx_addons() ) {
...
if ( ! function_exists( 'tornados_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'tornados_trx_addons_tiny_mce_style_formats' );
	function tornados_trx_addons_tiny_mce_style_formats( $style_formats ) {
		...
		if ( false && is_array( $style_formats ) && count( $style_formats ) > 0 ) {
			foreach ( $style_formats as $k => $v ) {
				if ( esc_html__( 'List styles', 'tornados' ) == $v['title'] ) {
					$style_formats[ $k ]['items'][] = array(
						'title'    => esc_html__( 'Arrow', 'tornados' ),
						'selector' => 'ul',
...
if ( ! function_exists( 'tornados_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'tornados_filter_allow_override_header_image', 'tornados_trx_addons_allow_override_header_image' );
	function tornados_trx_addons_allow_override_header_image( $allow ) {
		return is_single()
				&& (
					tornados_is_team_page()
					|| tornados_is_cars_page()
					|| tornados_is_cars_agents_page()
					|| tornados_is_properties_agents_page()
					)
				? false
				: $allow;
	}
...
if ( ! function_exists( 'tornados_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size', 'tornados_trx_addons_thumb_size', 10, 2 );
	function tornados_trx_addons_thumb_size( $thumb_size = '', $type = '' ) {
		      //Uncomment next lines to change team members images (default is 'avatar')
		      if ($type == 'team-short')
		          $thumb_size = tornados_get_thumb_size('alternate');
		return $thumb_size;
	}
}
...
if ( ! function_exists( 'tornados_trx_addons_meta_box_fields' ) ) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'tornados_trx_addons_meta_box_fields', 10, 2 );
	function tornados_trx_addons_meta_box_fields( $mb, $post_type ) {
		if ( defined( 'TRX_ADDONS_CPT_TEAM_PT' ) && TRX_ADDONS_CPT_TEAM_PT == $post_type ) {
			if ( ! isset( $mb['email'] ) ) {
				$mb['email'] = array(
					'title'   => esc_html__( 'E-mail', 'tornados' ),
					'desc'    => wp_kses_data( __( "Team member's email", 'tornados' ) ),
					'std'     => '',
					'details' => true,
					'type'    => 'text',
				);
...
if ( ! function_exists( 'tornados_filter_get_list_menu_hover' ) ) {
    add_filter( 'trx_addons_filter_get_list_menu_hover', 'tornados_filter_get_list_menu_hover' );
    function tornados_filter_get_list_menu_hover( $list ) {
        unset( $list['fade_box'] );
...
if ( ! function_exists( 'tornados_filter_get_list_input_hover' ) ) {
    add_filter( 'trx_addons_filter_get_list_input_hover', 'tornados_filter_get_list_input_hover' );
    function tornados_filter_get_list_input_hover( $list ) {
        unset( $list['accent'] );
        unset( $list['path'] );
        unset( $list['jump'] );
