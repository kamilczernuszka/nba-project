...
( function( $ ) {
	...
	var $style = $( '#tornados-customizer-inline-css' ),
		api    = wp.customize;

	// Prepare inline styles in the preview window
	if ( $style.length == 0 ) {
		var style_container = '<' + 'style type="text/css" id="tornados-customizer-inline-css" />',
			$theme_css      = $( '#tornados-main-inline-css' );
		if ($theme_css.length == 0) {
			$theme_css = $( '#tornados-main-css' );
		}
		if ($theme_css.length == 0) {
		...
		$style = $( '#tornados-customizer-inline-css' );
	}
			...
			api.preview.bind(
				'refresh-other-controls', function( obj ) {
					...
					if (id.indexOf( 'body_style' ) == 0) {
						...
						$( '.sc_layouts_logo .logo_text' ).html( tornados_prepare_macros( val ) );

					} else if (id == 'blogdescription') {
						$( '.sc_layouts_logo .logo_slogan' ).html( tornados_prepare_macros( val ) );

					} else if (id == 'copyright') {
						$( '.copyright_text' ).html( tornados_prepare_macros( val ) );
						...
						if (id == 'front_page_bg_image') {
							...
							if (color_type == 'none') {
								...
								color = typeof tornados_color_schemes[scheme] != 'undefined' ? tornados_color_schemes[scheme]['colors']['bg_color'] : '';
							}
							$( '.front_page_section_' + section + '_inner' ).css( 'background-color', color == '' ? 'transparent' : ( mask == 1 ? color : tornados_hex2rgba( color, mask ) ) );
							...
							.height( 'auto' )
							.toggleClass( 'tornados-full-height sc_layouts_flex sc_layouts_columns_middle', val );
							$( window ).trigger( 'resize' );
						}
					}
				}
