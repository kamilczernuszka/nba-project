<div class="front_page_section front_page_section_team<?php
	$tornados_scheme = tornados_get_theme_option( 'front_page_team_scheme' );
	if ( ! tornados_is_inherit( $tornados_scheme ) ) {
		echo ' scheme_' . esc_attr( $tornados_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( tornados_get_theme_option( 'front_page_team_paddings' ) );
?>"
		<?php
		$tornados_css      = '';
		$tornados_bg_image = tornados_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$tornados_anchor_icon = tornados_get_theme_option( 'front_page_team_anchor_icon' );
	$tornados_anchor_text = tornados_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( tornados_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$tornados_css      = '';
			$tornados_bg_mask  = tornados_get_theme_option( 'front_page_team_bg_mask' );
			$tornados_bg_color_type = tornados_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $tornados_bg_color_type ) {
				$tornados_bg_color = tornados_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$tornados_caption = tornados_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $tornados_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $tornados_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post( $tornados_caption ); ?></h2>
				<?php
			}

			// Description (text)
			$tornados_description = tornados_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $tornados_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $tornados_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post( wpautop( $tornados_description ) ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
				...
				if ( ! tornados_exists_trx_addons() ) {
					...
					tornados_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
				}
			}
			?>
			</div>
