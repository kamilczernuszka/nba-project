	...
	function() {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="tornados_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.tornados_post_editor' );
					if ( link.length > 0 ) {
						tornados_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.tornados_post_editor', function(e) {
					tornados_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('tornados_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function tornados_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
